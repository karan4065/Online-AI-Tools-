...
const handleDelete = async (id) => {
  ...
  return !loading ? (
    <div className='flex-1 h-full flex flex-col gap-4 p-6'>
      <h1 className='text-xl text-slate-600 font-semibold ml-2'>Creations History</h1>
      {creations.length > 0 ? (
         <div className='bg-white h-full w-full rounded-xl overflow-y-scroll'> 

            {creations.map((creation,index)=>(
             <div key={index} className='relative group inline-block pl-3 pt-3 w-full sm:max-w-1/2 lg:max-w-1/3'>
              <img src={creation.content} alt="" className='w-full h-full object-cover rounded'/>

              <div className='absolute bottom-0 top-0 right-0 left-3 flex gap-2 items-end justify-between group-hover: p-3 group-hover:bg-gradient-to-b from-transparent to-black/70 text-white rounded-lg'>
              ...
              <div className='flex gap-2 items-center'>
                <Download
                  onClick={() => handleDownload(creation.content, `creation-${creation.id}.jpg`)}
                  className="w-6 h-6 text-white cursor-pointer hover:scale-110"
                />

                <Trash
                  onClick={() => handleDelete(creation.id)}
                  className="w-6 h-6 text-white-400 cursor-pointer hover:scale-110"
                />
              </div>
            </div>
        ...
        <div className='flex justify-center items-center h-full'>
          <span className='text-xl'>No Creation Created  !!</span>
        </div>
      )}
