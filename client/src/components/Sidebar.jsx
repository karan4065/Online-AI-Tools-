...
import { Contact, Eraser, FileText, Hash, House, Image, LogOut, Scissors, SquarePen, Users } from 'lucide-react';
...
const navItems = [
    ...
    {to:'/ai/community', label : 'Community' , Icon:Users },
     {to:'/ai/contact-page', label : 'Contact' , Icon:Contact },
]
            ...
            <div className='px-6 mt-5 text-sm text-gray-600 font-medium'>
                {navItems.map(({to,label,Icon})=>(
                    <NavLink key={to} to={to} end={to === '/ai'} onClick={()=>
                        setSidebar(false)} className={({isActive})=>`px-3.5 py-2.5 flex items-center gap-3 rounded ${isActive ? 'bg-gradient-to-r z from-[#1841f8] to-[#00bbff] text-white' : ''}`} >
                            {({isActive})=>(
                                <>
