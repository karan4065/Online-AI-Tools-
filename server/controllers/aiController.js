...
export const generateImage = async (req, res) => {
  try {
    ...
    const base64Image = `data:image/png;base64,${Buffer.from(clipdropResponse.data, "binary").toString("base64")}`;
    ...
    const message = error.code === "ECONNABORTED"
      ? "Request timed out. Try again with a simpler prompt."
...
export const removeImageBackground = async (req, res) => {
  try {
    ...
    if (!image) return res.status(400).json({ success: false, message: "Image file is required." });
...
export const removeImageObject = async (req, res) => {
  try {
    ...
    if (!object) return res.status(400).json({ success: false, message: "Object name is required." });
    if (!image) return res.status(400).json({ success: false, message: "Image file is required." });
...
export const resumeReview = async (req, res) => {
  try {
    ...
    if (resume.size > 5 * 1024 * 1024) {
      return res.status(400).json({ success: false, message: "Resume file size must be less than 5MB." });
    }
    ...
    const content = response?.choices?.[0]?.message?.content;
    if (!content) throw new Error("AI failed to generate feedback.");
